...
FlightDetector::FlightDetector() : nh_("toland")
{
  ...
  {
    lrf_topic = "/lrf";
    ROS_WARN_STREAM("No LRF topic defined, using " << lrf_topic << std::endl);
  }
  ...
  ROS_INFO_STREAM("Parameter summary as loaded by toland"
                      ...
                      << imu_topic << "\n\tlrf_topic [string]:         " << lrf_topic << std::endl;);
...
void FlightDetector::imuCallback(const sensor_msgs::Imu::ConstPtr& msg)
{
  ...
  if ((meas.timestamp - imu_data_buffer_.begin()->timestamp) > k_sensor_readings_window_s_)
  {
    ...
    auto it = std::find_if(imu_data_buffer_.begin(), imu_data_buffer_.end(),
                           [&Dt](ImuData_t meas) { return meas.timestamp >= Dt; });
...
void FlightDetector::lrfCallback(const sensor_msgs::Range::ConstPtr& msg)
{
  ...
  // reject zero value measurements
  if (meas.range > 1e-1)
  {
    // Push measurement into buffer
    ...
    if ((meas.timestamp - lrf_data_buffer_.begin()->timestamp) > k_sensor_readings_window_s_)
    {
      ...
      // Get iterator to first element that has to be kept (timestamp > Dt (meas.timestamp - timestamp < window))
      auto it = std::find_if(lrf_data_buffer_.begin(), lrf_data_buffer_.end(),
                             [&Dt](LrfData_t meas) { return meas.timestamp >= Dt; });
...
bool FlightDetector::takeoffHandler(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  ...
  // INFO(scm): only use provide information with LRF if we have received measurements from LRF
